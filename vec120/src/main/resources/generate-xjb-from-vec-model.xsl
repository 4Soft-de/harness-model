...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="/">
        <jxb:bindings node="/xs:schema" schemaLocation="{concat('vec_',$VEC_VERSION,'.xsd')}" version="2.1"  extensionBindingPrefixes="xjc">
            ...
            <jxb:schemaBindings>                   
                <jxb:package name="com.foursoft.vecmodel.vec120" />
    ...
    <xsl:template match="packagedElement[@xmi:type='uml:Class']" mode="create-parents">
        <nav:parent>
            ...
            <xsl:attribute name="schema-type"><xsl:apply-templates select="." mode="create-name"/> </xsl:attribute>
        </nav:parent>
    </xsl:template>
    ...
    <xsl:template match="ownedAttribute" mode="create-property">
        ...
        <jxb:bindings>
            ...
            <nav:ext-reference>
                <xsl:attribute name="schema-type"><xsl:apply-templates select="$type" mode="create-name"/></xsl:attribute>
                <xsl:attribute name="inverse">ref<xsl:apply-templates select=".." mode="create-name"/> </xsl:attribute>               
            </nav:ext-reference>
        </jxb:bindings>
